...
const validationSchema = computed(() => toTypedSchema(companySchema(t)))

const { handleSubmit, errors, resetForm, defineField, setFieldError } = useForm<CompanyFormData>({
...
const onSubmit = handleSubmit(async (values) => {
	const response =
		props.type === 'create'
			...
			: await companyService.updateCompany(props.company.id, values)
			...
			for (const [field, fieldErrors] of Object.entries(response.errors)) {
				if (Array.isArray(fieldErrors)) {
					const messages = fieldErrors.map((error) =>
						t(`codes.${error.code}`, { param: t(`message.company.${field}`) }),
					)

					setFieldError(field as keyof CompanyFormData, messages.join(', '))
				}
			}
		}
...
const communeWardFormSchema: FormSchema = {
	fieldList: [
		{
			type: 'text',
			label: t('message.company.name_vn'),
			id: 'name_vn',
			required: true,
		...
		{
			type: 'text',
			label: t('message.company.name_en'),
			id: 'name_en',
			required: true,
...
<template>
	<FormFields :formSchema="communeWardFormSchema" @submit="onSubmit" id="company-form"></FormFields>
	<div class="flex justify-end gap-2 mt-4">
		<Button type="button" :label="t('button.cancel')" icon="pi pi-times" @click="onCancel" severity="secondary" />
		<Button type="submit" :label="t('button.save')" icon="pi pi-check" form="company-form" />
	</div>
</template>
